...
Pay Me Time is a proposed native iPhone commitment app. It gives people a configurable amount of free daily time in selected distracting apps, then makes additional access consume a deliberately tiny prepaid balance.
...
Five shipping targets compile and sign with explicit development profiles, Family Controls, and the shared App Group: the containing app, Device Activity Monitor, Shield Configuration, Shield Action, and Device Activity Report. The daily threshold and atomic debit/unshield/re-shield path are implemented and installed for physical verification. StoreKit delivery, Family Controls distribution approval, and the complete on-device behavior matrix remain release gates. Named Simulator fixtures are explicitly labeled as demo data.
